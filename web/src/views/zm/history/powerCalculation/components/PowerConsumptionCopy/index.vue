...
import {
  calculationZoneDay,
  calculationZoneMonth,
  calculationZoneQuarter,
  calculationZoneWeek, calculationZoneYear
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      if (this.type === 0) {
        calculationZoneDay(this.queryParams).then((response) => {
          this.demoList = response.rows
          this.total = response.total
          this.loading = false
        })
      } else if (this.type === 1) {
        calculationZoneWeek(this.queryParams).then((response) => {
          this.demoList = response.rows
          this.total = response.total
          this.loading = false
        })
      } else if (this.type === 2) {
        calculationZoneMonth(this.queryParams).then((response) => {
          this.demoList = response.rows
          this.total = response.total
          this.loading = false
        })
      } else if (this.type === 3) {
        calculationZoneQuarter(this.queryParams).then((response) => {
          this.demoList = response.rows
          this.total = response.total
          this.loading = false
        })
      } else if (this.type === 4) {
        calculationZoneYear(this.queryParams).then((response) => {
          this.demoList = response.rows
          this.total = response.total
          this.loading = false
        })
