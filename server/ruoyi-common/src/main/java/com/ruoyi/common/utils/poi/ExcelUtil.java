...
public class ExcelUtil {
    ...
    public static <T> Map<String, Object> importExcelByHead(InputStream is, Class<T> clazz, boolean isValidate) {
        DefaultExcelListener<T> listener = new DefaultExcelListener<>(isValidate);
        EasyExcel.read(is, clazz, listener).sheet().doRead();
        Map<String, Object> map = new HashMap<>();
        map.put("listener", listener.getExcelResult());
        map.put("head", listener.headMap);
        return map;
    }
     ...
     * @param os           输出流
     */
    private static void exportTemplateMultiSheet(List<Map<String, Object>> data, String templatePath, OutputStream os) {
        ClassPathResource templateResource = new ClassPathResource(templatePath);
        ExcelWriter excelWriter = EasyExcel.write(os)
            .withTemplate(templateResource.getStream())
            .autoCloseStream(false)
